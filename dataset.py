...
class Dataset:
    def __init__(self, train_list, test_list):
        ...
        with open(train_list) as f:
            ...
            for l in lines:
                ...
                #self.train_label.append(float(items[1]))
                for j in range(1,self.n_classes):
                    self.train_label.append(float(items[j]))
        ...
        with open(test_list) as f:
            ...
            for l in lines:
                ...
                #self.test_label.append(float(items[1]))
                for j in range(1,self.n_classes):
                    self.test_label.append(float(items[j]))
    ...
    def next_batch(self, batch_size, phase):
        ...
        # Expand labels
        one_hot_labels = np.zeros((batch_size, self.n_classes),dtype=float)
        for i, label in enumerate(labels):
            row_id= i//self.n_classes + 1
            col_id= i%self.n_classes
            one_hot_labels[row_id][col_id] = label#13.1#label[j]
        return images, one_hot_labels
